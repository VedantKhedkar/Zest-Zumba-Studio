          ...
          {packages.map((pkg, index) => (
            <div 
              ...
              className={clsx(
                ...
                // COLOR LOGIC:
                // Middle (Popular) -> Purple Gradient
                // Sides -> Yellow Gradient
                pkg.isPopular 
                  ? "bg-gradient-to-b from-purple-100 to-white border-purple-200 shadow-2xl shadow-purple-900/10 z-10" 
                  : "bg-gradient-to-b from-yellow-50 to-white border-yellow-100 shadow-lg text-slate-800 hover:border-yellow-200"
              )}
            >
              {/* Popular Badge */}
              {pkg.isPopular && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-yellow-400 text-purple-900 text-[10px] font-black uppercase py-1.5 px-4 rounded-full shadow-md tracking-widest">
                  Best Value
                </div>
              )}

              <div className="text-center mb-8 mt-2">
                <h3 className={clsx("text-xl font-black mb-4 font-heading uppercase tracking-wide", pkg.isPopular ? "text-purple-700" : "text-slate-700")}>
                    {pkg.title}
                </h3>
                <div className="flex items-start justify-center text-slate-800">
                  <span className="text-xl font-bold mt-2 text-slate-400">₹</span>
                  <span className={clsx("text-5xl font-black tracking-tighter mx-1", pkg.isPopular ? "text-purple-900" : "text-slate-900")}>
                    {pkg.price}
                  </span>
              ...
              <button 
                className={clsx(
                  "w-full py-3.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all duration-300 shadow-sm",
                  pkg.isPopular 
